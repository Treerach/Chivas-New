...
def bot(op):
    try:
        ...
        if op.type == 25:
            ...
            elif ("IaN1 gn " in msg.text):
                if msg.toType == 2:
                    X = cl.getGroup(msg.to)
                    X.name = msg.text.replace("IaN1 gn ","")
                    ki.updateGroup(X)
                else:
                    ki.sendText(msg.to,"It can't be used besides the group.")
            elif ("IaN2 gn " in msg.text):
                if msg.toType == 2:
                    X = cl.getGroup(msg.to)
                    X.name = msg.text.replace("IaN2 gn ","")
                    kk.updateGroup(X)
                else:
            ...
            elif "IaN1 kick " in msg.text:
                midd = msg.text.replace("IaN1 kick ","")
                ki.kickoutFromGroup(msg.to,[midd])
            elif "IaN2 kick " in msg.text:
                midd = msg.text.replace("IaN2 kick ","")
            ...
            elif "IaN1 invite " in msg.text:
                midd = msg.text.replace("IaN1 invite ","")
                ki.findAndAddContactsByMid(midd)
                ki.inviteIntoGroup(msg.to,[midd])
            elif "IaN2 invite " in msg.text:
                midd = msg.text.replace("IaN2 invite ","")
